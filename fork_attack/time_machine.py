...
cve_definitions = pd.read_csv("../data/cve_definitions.csv")
cwe_definitions = pd.read_csv("../data/cwe_definitions.csv")

dependabot_result = pd.read_csv("../data/dependabot_result.csv")
dependabot_python_data = dependabot_result.loc[dependabot_result['security_vulnerability.package.ecosystem'] == "pip"]

code_analysis_result = pd.read_csv("../data/code_analysis_result.csv")
...
cve_definitions.to_csv(f"../data/cve_definitions_lib.csv")
...
df_result_agg_cve.to_csv(f"../data/cve_definitions_lib_agg.csv")
...
df_all_agg['libraries'] = df_all_agg['libraries'].apply(lambda x: ', '.join(sorted(list(set(x)))) if isinstance(x, list) else [])

df_all_agg['repos'] = df_all_agg['repos'].apply(
    lambda x: list(map(str.lower, x)) if isinstance(x, list) else [])
df_all_agg['repos'] = df_all_agg['repos'].apply(lambda x: ', '.join(sorted(list(set(x)))) if isinstance(x, list) else [])


df_all_agg.to_csv("../data/all_definitions_agg_libs.csv")
